...
```{r}
dirname <- "/input/"
counts_matrix_filename = paste0(dirname,"J11987_002_TA_/raw_feature_bc_matrix/")
...
# Housekeeping genes
```{r}
Genes<-read.csv('~/References/Housekeeping_Hs.csv', head=T)
hkgenes <- as.vector(Genes$V1)
...
```{r}
H013_control <- subset(H013_control, subset = nFeature_RNA < 5000 & nFeature_RNA > 200 & percent.mt < 25 & percent.hk > 10)
dim(H013_control)
```
...
# 3.3.Gene ontology enrichment analysis (clusterProfiler)
```{r}
